...
static void *libjingoshelper = nullptr;
...
QQuickWindowPrivate::QQuickWindowPrivate()
    ...
    , startJingosHelper(nullptr)
    , stopJingosHelper(nullptr)
    , frameSynchronousEvents(nullptr)
    , takeTouchEventFromCache(nullptr)
    , jCompressTouchEvent(nullptr)
    , mergeTouchEvent(nullptr)
    , pushTouchEventToCache(nullptr)
    , isCacheTouchEmpty(nullptr)
{
#if QT_CONFIG(quick_draganddrop)
    ...
    if (libjingoshelper == nullptr)
        libjingoshelper = dlopen("/opt/jingos/libjingoshelper.so", RTLD_LAZY);
        if (libjingoshelper == nullptr)
        fprintf(stderr, "dlopen: %s ",  dlerror());

    if (libjingoshelper) {
        startJingosHelper = (void* (*)(void))dlsym(libjingoshelper, "startJingosHelper");
	if (startJingosHelper == nullptr)
            fprintf(stderr, "dlsym: %s ",  dlerror());
        stopJingosHelper = (void(*)(void *))dlsym(libjingoshelper, "stopJingosHelper");
        frameSynchronousEvents = (bool(*)(void *))dlsym(libjingoshelper, "frameSynchronousEvents");
        takeTouchEventFromCache = (QTouchEvent *(*)(void *))dlsym(libjingoshelper, "takeTouchEventFromCache");
        jCompressTouchEvent = (bool(*)(void *,QTouchEvent *))dlsym(libjingoshelper, "compressTouchEvent");
        mergeTouchEvent = (bool(*)(void *,QTouchEvent *))dlsym(libjingoshelper, "mergeTouchEvent");
        pushTouchEventToCache = (void(*)(void *,QTouchEvent *))dlsym(libjingoshelper, "pushTouchEventToCache");
        isCacheTouchEmpty = (bool(*)(void *))dlsym(libjingoshelper, "isCacheTouchEmpty");
    }

    if (startJingosHelper) {
       jhelper = startJingosHelper();
    } else {
        fprintf(stderr, "libingoshelper is incompatible or missing.");
    }
...
void QQuickWindowPrivate::init(QQuickWindow *c, QQuickRenderControl *control)
{
    ...
    //void (*startJingosHelper)(void) = NULL;
    //void (*stopJingosHelper)(void *) = NULL;
    //bool (*frameSynchronousEvents)(void *) = NULL;
    //QTouchEvent * (*takeTouchEventFromCache)(void *) = NULL;
    //bool (*compressTouchEvent)(void *,QTouchEvent *) = NULL;
    //bool (*mergeTouchEvent)(void *,QTouchEvent *) = NULL;
    //void (*pushTouchEventToCache)(void *,QTouchEvent *) = NULL;
    //bool (*isCacheTouchEmpty)(void *) = NULL;
...
void QQuickWindowPrivate::deliverDelayedTouchEvent()
{
    if (jhelper) {
        QTouchEvent * t = NULL;
        if (takeTouchEventFromCache)
            t = takeTouchEventFromCache(jhelper);

        if (t) {
            QScopedPointer<QTouchEvent> e(t);
...
bool QQuickWindowPrivate::compressTouchEvent(QTouchEvent *event)
{
    ...
    if (jhelper) {
         bool compress = false;
        if (jCompressTouchEvent) {
           compress = jCompressTouchEvent(jhelper, event);
        }
        if (compress) {
             if (renderControl)
        ...
        bool merge = false;
        if (mergeTouchEvent) {
            merge = mergeTouchEvent(jhelper, event);
        }
        ...
        if (pushTouchEventToCache) {
            pushTouchEventToCache(jhelper, event);
        }
...
void QQuickWindowPrivate::handleTouchEvent(QTouchEvent *event)
{
	...
	if (jhelper) {
   	    while (!isCacheTouchEmpty(jhelper))
                deliverDelayedTouchEvent();   
        } else {
            if (delayedTouch)
                deliverDelayedTouchEvent();   
...
void QQuickWindowPrivate::flushFrameSynchronousEvents()
{
    ...
    if (jhelper) {
        bool sync = false;
        if (frameSynchronousEvents)
            sync = frameSynchronousEvents(jhelper);
        if (sync) {
            deliverDelayedTouchEvent();
            flushTouch = true;
...
void QQuickWindowPrivate::deliverMatchingPointsToItem(QQuickItem *item, QQuickPointerEvent *pointerEvent, bool handlersOnly)
{
    ...
    if (sendFilteredPointerEvent(pointerEvent, item))
     ...
     //qDebug()<<"[PERFETTO]"<<__func__<<" - actually delivering  " << touchEvent.data();
